 ...
 */
class PayPalHelper extends
    OmnipayHelperAbstract
{
     ...
     */
    public function translatePayPalPayerID(
        PayPalCompleteRequestData $request_data,
        string $payer_id
    ): void {

        // Set the payer ID field into the request data
        $request_data->setMainParameter(
            'payerId',
            $payer_id
        );
    }
     ...
     * @throws \Exception
     */
    public function newOmnipayGatewayOffsiteCustomerFromResponse($response
    ): OmnipayGatewayOffsiteCustomer {
        ...
        $address = (isset($payer['billing_address']))
            ? $payer['billing_address'] : $payer['shipping_address'];

        // Return a new OmnipayGatewayOffsiteCustomer using data from the PayPal response
        return new OmnipayGatewayOffsiteCustomer(
            $payer['first_name'],
            $payer['last_name'],
            $payer['email'],
            $address['line1'],
